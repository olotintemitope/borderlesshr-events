                ...
                <div class="col">
                    ...
                    <?php } else { ?>
                        <div class="card h-100">
                            <img src="<?php echo $viewPath . "../../uploads/" . $event['img_cover'] ?>"
                                 class="card-img-top"
                            ...
                            <div class="card-body">
                                ...
                                <p><?php if ($isClosed) {
                                        ...
                                        echo "<span class='badge bg-light text-dark'>" . (new \DateTime($event['registration_deadline_date']))->format('Y-m-d') . "</span>";
                                    } ?></p>
                                <p>
                                    <?php if (!$isClosed && (new \DateTime()) >= (new \DateTime($event['date_opened']))) { ?>
                                        <?php if (!$applied) { ?>
                                            <a href="/events/apply/<?php echo strtolower(str_replace(" ", "-", $event['title'])) . '--' . $event['id'] ?>"
                                               class="btn btn-warning text-white">Apply</a>
                                        <?php } else { ?>
                                            <button type="button" class="btn btn-default">Applied</button>
                                        <?php } ?>
                                    <?php } else { ?>
